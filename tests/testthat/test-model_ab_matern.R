...
test_that("`model_ab_matern` gives the same likelihood as the one calculated in R under different parametrizations of shape parameter", {
  ...
  for (ii in 1:n_tests){
    ...
    # Unconstrained s
    sim_res <- test_sim(random = c("a", "b"), kernel = "matern", reparam_s = "unconstrained")
    nll_tmb <- calc_tmb_nll(sim_res)
    expect_equal(sim_res$nll_r, nll_tmb)
    # Negative s
    sim_res <- test_sim(random = c("a", "b"), kernel = "matern", reparam_s = "negative")
    nll_tmb <- calc_tmb_nll(sim_res)
    expect_equal(sim_res$nll_r, nll_tmb)
    # s=0
    sim_res <- test_sim(random = c("a", "b"), kernel = "matern", reparam_s = "zero")
    nll_tmb <- calc_tmb_nll(sim_res)
    expect_equal(sim_res$nll_r, nll_tmb)
  }
})
