...
![Posterior mean estimates of the 10% return level $z_{10}(\xx)$ plotted against the true values at different locations.\label{fig:sim-return-level}](sim-return-level.pdf){width=28%}
...
train_fit <- spatialGEV_fit(y = y_train, X = locs_train, random = "ab", 
                          init_param = list(a = a[-test_ind], log_b = logb[-test_ind], s = logs, 
                                            log_sigma_a = 1, log_ell_a = 5, 
                                            log_sigma_b = 1, log_ell_b = 5),
                          reparam_s = "positive", silent = TRUE)
