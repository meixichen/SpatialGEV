...
#' @export
map_plot <- function(y, x, title, ylim, legend.shrink=0.5, legend.horizontal=FALSE){
  parset = par(mar=c(0,0,0,5)+2)
  if (missing(ylim)) ylim <- range(y)
  maps::map("world", mar=rep(0,4)+1, xlim=range(x[,1]), ylim=range(x[,2]), col="white", fill=T, lwd=0.05)
  fields::image.plot(legend.only=T, zlim=ylim, col=viridisLite::viridis(6), horizontal = legend.horizontal,
                     legend.shrink = legend.shrink, cex = 1.4) # for setting up the legend
  ...
  maps::map("world", mar=rep(0,4)+1, xlim=range(x[,1]), ylim=range(x[,2]), lwd=1.25, add=T)
  maps::map("state", mar=rep(0,4)+1, xlim=range(x[,1]),ylim=range(x[,2]), lwd=1.25, add=T)
}
