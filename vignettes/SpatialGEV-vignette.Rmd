...
\end{equation}
***SpatialGEV*** supports three types of kernel functions $k(\xx,\xx')$:

- Exponential kernel $k(\xx,\xx')=\sigma^2e^{-\frac{|\xx-\xx'|}{\ell}}$, in which case $\tth_u=(\sigma_u^2,\ell_u)$ for $u\in \{a,b,s\}$. 

- Matérn kernel $k(\xx,\xx')=\sigma^2 \frac{2^{1-\nu}}{\Gamma(\nu)} (\kappa |\xx-\xx'|)^\nu \mathcal{K}_\nu(\kappa|\xx-\xx'|)$, where $\mathcal{K}_\nu(\cdot)$ is the modified Bessel function of the second kind and $\nu$ is typically fixed. In this case, $\tth_u=(\sigma_u^2,\kappa_u)$ for $u\in \{a,b,s\}$. 

- An SPDE approximation to the above Matérn kernel described in @lindgren-etal11, in which case $\tth_u=(\sigma_u^2,\kappa_u)$ for $u\in \{a,b,s\}$. 
...
```{r sim1-plot-abs, fig.width=14, fig.height=5, out.width="100%"}
par(mfrow=c(1,3))
...
```{r sim-1plot-y, fig.width=12, fig.height=6, out.width= "100%"}
set.seed(123)
n_sam <-8
...
```{r sim1-pos-plots, fig.width=12, fig.height=4, out.width= "100%"}
par(mfrow=c(1,3))
...
```{r sim2-plot-ab, fig.width=8, fig.height=4.5, out.width="50%"}
a <- simulatedData$a
...
```{r sim2-pred-plot, fig.width=12, fig.height=6, out.width= "100%"}
par(mfrow=c(1,2))
...
```{r snow-display-raw-data, message=FALSE, fig.width=6, fig.height=5, out.width= "60%"}
library(dplyr)
#library(maps)
lon_range <- c(-96, -73)
lat_range <-  c(41.5, 55)
...
```{r snow-display-grid-data, fig.width=6, fig.height=5, out.width= "60%"}
maps::map(xlim = lon_range, ylim = lat_range)
...
```{r snow-return-plots, fig.width=10, fig.height=5, out.width= "60%", cache=FALSE}
par(mfrow=c(1,2))
