...
s(\xx) \mid \bm{\beta}_s, \tth_s &\sim \mathcal{GP}\big( \XX_b\bm{\beta}_s, k(\xx, \xx' \mid \tth_a) \big).
\end{aligned}
\end{equation}
...
sam <- spatialGEV_sample(fit, n_draw=2000, observation=T)
```

From the samples we can calculate the 50% and 10% return levels at all locations, which are the upper 50% and 10% quantiles of the extreme value distributions at these locations.
```{r}
y_draws <- sam$y_draws
y_return <- t(apply(y_draws, 2, 
                    quantile, probs=c(0.5, 0.9)))
```

Plotted below are the return levels.
```{r snow-plots, fig.width=5, fig.height=4, out.width= "60%"}
map_plot <- function(value){
  maps::map("world", xlim=lon_range, ylim = lat_range, col="white", fill=TRUE)
  fields::image.plot(legend.only = T, zlim=range(value),col=viridisLite::viridis(6), 
                     horizontal = TRUE, legend.shrink = 0.5,
                     cex=0.7, smallplot=c(0.4, 0.9, 0.1, 0.14))
  val = fields::color.scale(value, col=scales::alpha(viridisLite::viridis(6), 1))
  points(locs$cell_lon, locs$cell_lat, col=val, pch=15, cex=0.9)
}
map_plot(y_return[, "50%"])
title("50% Return levels")
map_plot(y_return[, "90%"])
title("10% Return levels")
```
